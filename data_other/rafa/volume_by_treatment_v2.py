...
beyond-nightlight treatment map (fig-map.csv, 0.005 degree cells), and:
  ...
  4. plots mean volume per cell over time, one line per bin, in levels and
     as level differences against the lowest intensity bin.
...
Usage:
    python volume_by_treatment_v2.py
    python volume_by_treatment_v2.py --presence-threshold 0.5 --weight binary
    python volume_by_treatment_v2.py --reference 0-3
"""
...
def main(folder, presence_threshold, weight, dpi, reference):
    ...
    for b in present:
        ...
        diff = s["mean_volume_m3"].values - ref_series.reindex(s["year"]).values
        if b == ref:
            axes[1].axhline(0, color="#999999", linewidth=0.8, linestyle="--")
        else:
            axes[1].plot(s["year"], diff, marker="o", color=colors[b],
