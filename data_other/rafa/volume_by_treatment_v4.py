# -*- coding: utf-8 -*-
"""
Built footprint area or volume over time by GiveDirectly treatment intensity.
...
beyond-nightlight treatment map (fig-map.csv, 0.005 degree cells), and:
  ...
  1. computes, per 4 m pixel that clears the presence threshold, footprint
     area as pixel_area and built volume as height x pixel_area,
  2. aggregates both to the same 0.005 degree grid the treatment map uses,
  3. joins on treatment intensity bin,
  4. plots the mean outcome per cell over time, one line per bin, in levels
     and as level differences against the lowest intensity bin, with
     confidence intervals on both.

v4: --outcome selects footprint area in m2 (the default) or built volume in
m3. Area is the closer analogue to the building footprint outcome in Huang,
Hsiang and Gonzalez-Navarro, whose headline estimate is 7.9 m2 per USD 1,000
transfer, so it is the comparable number.

v3: confidence intervals, from a spatial block bootstrap by default, since
neighbouring cells are strongly correlated and iid intervals would be far
too narrow.

v2: the right panel used to index everything to the first year. That was
wrong. 2016 is not a pre period, since transfers ran from mid 2014 to early
2017, so the base year is itself partly treated. Dividing by it absorbs any
effect that had already landed and, worse, a genuine positive effect on 2016
volume mechanically pushes the treated bins' index down, which can flip the
sign. Level differences never divide by an outcome.

Outputs, written next to the inputs:
  siaya_volume_by_cell.csv      cell x year panel, area and volume
  siaya_volume_by_bin.csv       bin x year summary with interval bounds
...
Usage:
    python volume_by_treatment_v4.py
    python volume_by_treatment_v4.py --outcome volume_m3
    python volume_by_treatment_v4.py --ci iid --n-boot 2000
"""
...
def compute_cis(panel, present, years, ref, method, n_boot, block_deg,
    ...
    if method == "iid":
        levels, diffs = {}, {}
        stats = (panel.groupby(["treatment_intensity", "year"])[outcome]
                 .agg(["mean", "std", "size"]))
        z = 1.959963985
        for b, y in keys:
    ...
    grouped = work.groupby(["block", "treatment_intensity", "year"])[outcome]
...
def main(folder, presence_threshold, weight, dpi, reference,
         ci_method, n_boot, block_deg, alpha, seed, outcome):
    ...
    # --- summary by bin -----------------------------------------------------
    unit_name = "footprint area" if outcome == "built_area_m2" else "volume"
    unit_sym = "m2" if outcome == "built_area_m2" else "m3"

    summary = (panel
               .groupby(["treatment_intensity", "year"], as_index=False)
               .agg(mean_outcome=(outcome, "mean"),
                    sd_outcome=(outcome, "std"),
                    total_outcome=(outcome, "sum"),
                    mean_volume_m3=("volume_m3", "mean"),
                    mean_built_area_m2=("built_area_m2", "mean"),
                    n_cells=(outcome, "size")))
    print(f"\nMean built {unit_name} per cell ({unit_sym}):")
    print(summary.pivot(index="year", columns="treatment_intensity",
                        values="mean_outcome").round(0).to_string())
    ...
    summary["diff_vs_ref"] = [
        ...
        for m, y in zip(summary["mean_outcome"], summary["year"])]
    ...
    axes[0].set_ylabel(f"Mean built {unit_name} per cell ({unit_sym})")
    axes[0].set_title(f"Levels, {int((1 - alpha) * 100)}% CI")
    axes[1].set_ylabel(f"Difference vs. {BIN_LABELS.get(ref, ref)} ({unit_sym})")
    ...
    axes[1].legend(title="Recipient households per cell", fontsize=9,
                   title_fontsize=9)
    fig.suptitle(f"Built {unit_name} by GiveDirectly treatment intensity, "
                 f"Siaya County", fontsize=13)
    fig.tight_layout()
...
if __name__ == "__main__":
    ...
    p.add_argument("--outcome", choices=["built_area_m2", "volume_m3"],
                   default="built_area_m2",
                   help="footprint area in m2, or built volume in m3")
    ...
    main(a.folder, a.presence_threshold, a.weight, a.dpi, a.reference,
         a.ci, a.n_boot, a.block_deg, a.alpha, a.seed, a.outcome)
