...
beyond-nightlight treatment map (fig-map.csv, 0.005 degree cells), and:
  ...
  4. plots mean volume per cell over time, one line per bin, in levels and
     as level differences against the lowest intensity bin, with confidence
     intervals on both.

v3: adds confidence intervals. Default is a spatial block bootstrap, which
resamples coarse blocks rather than individual cells, because neighbouring
cells are strongly correlated and treating them as independent would produce
intervals several times too narrow.

v2 note: the right panel used to index everything to the first year. That was
...
Usage:
    python volume_by_treatment_v3.py
    python volume_by_treatment_v3.py --ci iid
    python volume_by_treatment_v3.py --block-deg 0.03 --n-boot 2000
"""
...
def compute_cis(panel, present, years, ref, method, n_boot, block_deg,
    ...
    if method == "iid":
        levels, diffs = {}, {}
        stats = (panel.groupby(["treatment_intensity", "year"])["volume_m3"]
                 .agg(["mean", "std", "size"]))
        z = 1.959963985
        for b, y in keys:
    ...
    grouped = work.groupby(["block", "treatment_intensity", "year"])["volume_m3"]
...
def main(folder, presence_threshold, weight, dpi, reference,
    ...
    summary["diff_vs_ref"] = [
        ...
        for m, y in zip(summary["mean_volume_m3"], summary["year"])]
    ...
    axes[1].set_ylabel(f"Difference vs. {BIN_LABELS.get(ref, ref)} (m3)")
